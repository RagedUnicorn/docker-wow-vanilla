...
ENV \
  ...
  LIBSSL_1_0_DEV_VERSION=1.0.2n-1ubuntu5.2 \
...
# build server
COPY data/server "${WOW_HOME}"/server/

RUN mkdir "${WOW_HOME}"/server/build
...
RUN \
  set -ex && \
  tar xvzf "${WOW_HOME}"/*.tar.gz -C "${WOW_HOME}" && \
  rm -rf "${WOW_HOME}"/*.tar.gz
